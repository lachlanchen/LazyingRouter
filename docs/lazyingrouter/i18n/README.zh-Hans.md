...
LazyingRouter 是一个私有 AI API 聚合网关，用于统一管理用户账号、用户 API Key、额度、用量记录和上游模型服务路由。

它基于成熟的开源 New API 项目构建，目标是让 AgInTiFlow 和其他 LazyingArt 工具只需要登录 LazyingRouter，就可以通过统一的额度和模型路由访问 OpenRouter、Venice、GRSAI、Claude 兼容服务以及后续供应商。
...
详细操作说明见：[LazyingRouter operator guide](../README.md)。
